...
namespace H5Plugins
{
    ...
    public class ScheduleExport
    {
        ...
        {           
            ...
            string myreviewValue = ScheduleExportMVVM.MainView.ViewModel.MyReviewValue;
            ...
            IEnumerable<ScheduleExportViewModel> selecteds = ScheduleExportMVVM.MainView.MainViewModel.Where(ps => ps.IsCheckedScheduleName == true);
            ...
            {
                ScheduleExportMVVM.MainView.Close();
            ...
            else
            {
                ...
                string coverfilePath = @"V:\Projetos\2108-BIM\Desenvolvimento-BIM\00-TEMPLATES\07-LISTA DE MATERIAIS\XX-CAPAS\" + centroDeCusto + "-CAPA.XLS";
                ...
                if (coverFileInfor.Exists)
                {
                    ...
                    //Creating Schedule File in .xls foreach schedule in Revit UI File
                    try
                    {
                        foreach (ViewSchedule vs in SelectedScheduleList)
                        {
                            if (!vs.IsTemplate)
                            {
                                ...
                                vs.Export(directoryFinalName, scheduleName + myreviewValue + ".xls", opt);
                            }
                        }
                    }
                    catch
                    ...
                    using (var progressBar = new ProgressBar("Preparando as Listas de Materiais..."))
                    {
                        progressBar.RunSheets(worksheetsList, (Worksheet activeWorksheet) =>
                        {
                            if (activeWorksheet.Name != "Capa" && activeWorksheet.Name != "DADOS")
                            {
                                ...
                                int rowCount = 1;
                                //LISTA QUE CONTÉM AS LINHAS QUE SÂO AS DESCRIÇÕES DOS ELEMENTOS
                                List<int> descriptionRowList = new List<int>();

                                //DANDO MERGE NAS LINHAS QUE CONTÉM A DESCRIÇÃO DOS MATERIAIS
                                foreach (Range row in rows)
                                {
                                    try
                                    {
                                        Range firstCell = row.Cells[1, 1];
                                        Range lastCell = row.Cells[1, numberOfColumns];
                                        string firstCellValue = Convert.ToString(firstCell.Value);
                                        int numberOfCarachters = firstCellValue.Length;

                                        //CONFIGURANDO AS LINHAS QUE CONTÉM A DESCRIÇÃO DOS ITENS                                
                                        if (numberOfCarachters > 40)
                                        {
                                            descriptionRowList.Add(rowCount);
                                            activeWorksheet.get_Range(firstCell, lastCell).Style.HorizontalAlignment = XlHAlign.xlHAlignCenter;
                                            activeWorksheet.get_Range(firstCell, lastCell).Style.VerticalAlignment = XlHAlign.xlHAlignCenter;
                                            activeWorksheet.get_Range(firstCell, lastCell).Merge();
                                            activeWorksheet.get_Range(firstCell, lastCell).Cells.WrapText = true;
                                            activeWorksheet.get_Range(firstCell, lastCell).Rows.AutoFit();
                                            activeWorksheet.get_Range(firstCell, lastCell).Columns.AutoFit();
                                        }
                                        else
                                        {
                                        }
                                ...
                                Range lastCellinFirtsRow = activeWorksheet.Cells[1, numberOfColumns];
                                ...
                                activeWorksheet.get_Range(firstCellInFirstColumnRange, lastCellinFirtsRow).Font.FontStyle = "Bold";
                                activeWorksheet.get_Range(firstCellInFirstColumnRange, lastCellinFirtsRow).Borders.LineStyle = XlLineStyle.xlContinuous;
                                activeWorksheet.get_Range(firstCellInFirstColumnRange, lastCellinFirtsRow).Borders.Weight = XlBorderWeight.xlThin;
                                ...
                                activeWorksheet.get_Range(firstCellInFirstColumnRange, lastCellInLastColumnRange).Cells.Font.Name = "Arial";
                                activeWorksheet.get_Range(firstCellInFirstColumnRange, lastCellInLastColumnRange).Cells.Font.Size = "10";
                                ...
                                //AJUSTANDO A ALTURA DAS CÉLULAS QUE CONTÉM 
                                foreach (int item in descriptionRowList)
                                {
                                    if (item == 2)
                                    {
                                        activeWorksheet.Cells[item, 1].RowHeight = 100.00;

                                    }
                                    else if (item > 2)
                                    {
                                        activeWorksheet.Cells[item, 1].RowHeight = 100.00;
                                        //CONFIGURANDO AS LINHAS EM BRANCO                                
                                        activeWorksheet.Cells[item - 1, 1].RowHeight = 10.00;
                                        Range firstCell = activeWorksheet.Cells[item - 1, 1];
                                        Range lastCell = activeWorksheet.Cells[item - 1, numberOfColumns];
                                        activeWorksheet.get_Range(firstCell, lastCell).Merge();
                                        activeWorksheet.get_Range(firstCell, lastCell).Borders.LineStyle = XlLineStyle.xlLineStyleNone;
                                    }
                                }
                                ...
                                //FIXANDO A PRIMEIRA LINHA COMO CABEÇALHO DE TODAS AS FOLHAS DE IMPRESSÃO
                                activeWorksheet.PageSetup.PrintTitleRows = "$1:$1";

                                //DEFININDO A ÁREA DE IMPRESSÃO NA FOLHA (DESNECESSÁRIO)
                                //activeWorksheet.PageSetup.PrintArea = "$A$1:$H$20";                  
                            }
                ...
                else
                {
                    ...
                    TaskDialog.Show("ERRO!", "ERRO! O arquivo de Capa não foi encontrado. Certifique-se de que ele está no caminho correto e que o centro de custo é adequado." + Environment.NewLine + Environment.NewLine +
                        @"O caminho para o arquivo na pasta é:" + Environment.NewLine + Environment.NewLine + @"V:\Projetos\2108 - BIM\Desenvolvimento - BIM\00 - TEMPLATES\07 - LISTA DE MATERIAIS\XX - CAPAS\", tskbuttons, tskresult);
                }                
            }            
        }
