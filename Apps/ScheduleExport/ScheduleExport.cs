...
namespace H5Plugins
{
    ...
    {        
        ...
        public void Eletrica(Document doc)
        {           
            CheckExcellProcesses();            

            ScheduleExportMVVM scheduleExportmainView = H5AppExternalApplication.h5App.scheduleExportMVVM;
            ...
            IEnumerable<ScheduleExportViewModel> selecteds = scheduleExportmainView.MainViewModel.Where(ps => ps.IsCheckedScheduleName == true);
            ...
            TaskDialog.Show("Teste", centroDeCusto.ToString());
                    ...
                    using (var progressBar = new ProgressBar("Preparando as Listas de Materiais..."))
                    {
                        progressBar.RunSheets(worksheetsList, (Worksheet activeWorksheet) =>
                        {
                            if (activeWorksheet.Name != "Capa" && activeWorksheet.Name != "DADOS")
                            {
                                ...
                                foreach (Range row in rows)
                                {
                                    try
                                    {
                                        ...
                                        if (numberOfCarachters > 40)
                                        {
                                            ...
                                            activeWorksheet.get_Range(firstCell, lastCell).Style.HorizontalAlignment = XlHAlign.xlHAlignLeft;
                                ...
                                try
                                {
                                    //AJUSTANDO A ALTURA DAS CÉLULAS QUE CONTÉM AS DESCRIÇÕES
        ...
        public void Mecanica(Document doc)
        {
            CheckExcellProcesses();
                    ...
                    using (var progressBar = new ProgressBar("Preparando as Listas de Materiais..."))
                    {
                        progressBar.RunSheets(worksheetsList, (Worksheet activeWorksheet) =>
                        {
                            if (activeWorksheet.Name != "Capa" && activeWorksheet.Name != "DADOS")
                            {
                                ...
                                //activeWorksheet.VPageBreaks.Add(activeWorksheet.Range[]);

                                TaskDialog.Show("teste", "$" + firstcolumnNumberToLetter + "$"
                                    + firstrowNumber + ":$" + lastcolumnNumberToLetter + "$" + lastrowNumber);

                                //activeWorksheet.get_Adress(firstCellInFirstColumnRange, lastCellInLastColumnRange);
                                //activeWorksheet.PageSetup.PrintArea = "$A$1:$H$20";     
